...
class coAttention_alt(Layer):
    ...
    def __init__(self, dim_k, output_dim, precomp_features=False, **kwargs):
    ...
    def build(self, input_shape):
        ...
        if len(input_shape) != 2:
            raise ValueError('A Co-Attention_alt layer should be called on a '
                             'list of 3 inputs. '
                             'Got '+str(len(input_shape))+'inputs.')
        # print(input_shape)
        self.num_imgRegion = 1 if self.precomp_features else input_shape[0][1]
        self.seq_len = 1 if self.precomp_features else input_shape[1][1]
        ...
        super(coAttention_alt, self).build(input_shape)
    ...
    def call(self, x, mask=None):
        ...
        w_Vi_0 = K.reshape(w_Vi_0, [-1, num_imgRegion, dim_k])
        # shape=(batchSize,num_imgRegion,dim_k)
        w_Vt_0 = K.repeat(K.dot(tfeature, self.w_Dense_Vt_0), num_imgRegion)
        # shape=(batchSize,num_imgRegion,dim_k)
        Vi_Vt_0 = K.concatenate([w_Vi_0, w_Vt_0], axis=-1)
        # shape=(batchSize,num_imgRegion,2*dim_k)
        Hi = K.tanh(Vi_Vt_0)
        ...
        Hi_w_b = K.squeeze(K.dot(Hi, self.w_Dense_Pi_0), axis=-1) \
            + self.b_Dense_Pi_0  # shape=(batchSize,num_imgRegion)
        Pi = K.softmax(Hi_w_b)
        Pi = K.permute_dimensions(K.repeat(Pi, output_dim), (0, 2, 1))
        # shape=(batchSize,num_imgRegion,output_dim)
        Pi_Vi = Pi*ifeature
        Vi = K.sum(Pi_Vi, axis=1)  # shape=(batchSize,output_dim)

        # phase 1: image-guided text feature computation
        w_Vi_1 = K.repeat(K.dot(Vi, self.w_Dense_Vi_1), seq_len)
        # shape=(batchSize,seq_len,dim_k)
        w_Vt_1 = K.dot(K.reshape(tfeature_h, [-1, output_dim]),
                       self.w_Dense_Vt_1)  # shape=((batchSize*seq_len),dim_k)
        w_Vt_1 = K.reshape(w_Vt_1, (-1, seq_len, dim_k))
        # shape= (batchSize, seq_len, dim_k)
        Vi_Vt_1 = K.concatenate([w_Vi_1, w_Vt_1], axis=-1)
        # shape=(batchSize, seq_len, 2*dim_k)
        Ht = K.tanh(Vi_Vt_1)
        Ht_b = K.squeeze(K.dot(Ht, self.w_Dense_Pi_1), axis=-1) \
            + self.b_Dense_Pi_1   # shape=(batch_size, seq_len)
        Pt = K.softmax(Ht_b)
        Pt = K.permute_dimensions(K.repeat(Pt, output_dim), (0, 2, 1))
        # shape=(batchSize, seq_len, output_dim)
        Pt_Vt = Pt*tfeature_h
...
def myLossFunc(y_true, y_pred):
    check_numerics(y_pred, "")
    probs_log = -K.log(K.clip(y_pred, 1e-10, 1.0))
    loss = K.mean(K.sum(probs_log * cast(y_true, float32), axis=-1))
